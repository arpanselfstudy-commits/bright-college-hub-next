...
const generateRequestDescriptionSchema = z.object({
...
type GenerateRequestDescriptionInput = z.infer<typeof generateRequestDescriptionSchema>

// ---------------------------------------------------------------------------
// In-memory rate limiter (shared limit: 10 req/user/hour)
// ---------------------------------------------------------------------------
...
function buildPrompt(input: GenerateRequestDescriptionInput): string {
  return (
    'You are a helpful assistant for a college student marketplace called Bright College Hub.\n' +
    'Write a 2-3 sentence product request description for a student who is LOOKING TO BUY the following item:\n' +
    `- Product Name: ${input.name}\n` +
    `- Category: ${input.category}\n` +
    `- Budget: $${input.priceFrom} – $${input.priceTo}\n` +
    'Write in a polite, friendly tone as if the student is posting a "wanted" ad.\n' +
    'Do NOT mention the exact price range in the description.\n' +
...
export const POST = withErrorHandler(async (req: NextRequest) => {
  ...
  const data = validate(generateRequestDescriptionSchema, body)
    ...
    aiRes = await fetch('https://api.ai.cc/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a helpful assistant for a college student marketplace. Write concise, polite product request descriptions.',
          },
